...
class CTkPieChart(ctk.CTkLabel):
    ...
    def __init__(self, master, command=None, values={}, font_size=None, **kwargs):
        ...
        self.font_size = font_size or 80 
    ...
    def draw_pie_chart(self, *args):
        ...
        for value in self.values.values():
            ...
            # Adjust text position to be in the center of each slice
            text_radius = radians / 2  # This places the text halfway between the center and the border

            midpoint1_x = xn + text_radius * math.cos(math.radians(midpoint_angle))
            midpoint1_y = yn + text_radius * math.sin(math.radians(midpoint_angle))
    ...
    def add(self, tag, value, color=None, text_color=None, draw=True, font_size=60):
        
        if tag in self.values:
            self.update(tag, value, color, text_color, font_size)
            return
        ...
        self.values.update({tag:{'color': color, 'value': value, 'text_color': text_color, 'font_size': font_size}})
    ...
    def update(self, tag, value=None, color=None, text_color=None, font_size=None):
        if tag in self.values:
            if value:
            ...
            if font_size:
                self.values[tag]['fon_size'] = font_size
